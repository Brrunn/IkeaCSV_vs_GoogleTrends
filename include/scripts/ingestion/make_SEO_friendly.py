...
def process_batch(df, model_name, start, end):
    ...
    try:
        ...
        print(f"[ERROR] JSON non parsable pour le lot {start+1}-{end} : {e}\nContenu : {text!r}")

    return mapping


def enrich_csv_keywords_parallel(csv_path, output_csv, model_name=DEFAULT_MODEL_NAME, batch_size=DEFAULT_BATCH_SIZE):
...
if __name__ == "__main__":
    enrich_csv_keywords_parallel(
        "include/data/raw/ikea.csv",
        "include/data/processed/ikea_with_keywords.csv"
    )
